"""
Enhanced Factor Analysis with Value Factor
==========================================
Analyzes factor coefficients including the value factor from the QVM engine
...
class EnhancedFactorAnalyzer:
    def __init__(self):
        ...
        self.qvm_engine = QVMEngineV2Enhanced()
    ...
    def get_value_factors(self, analysis_date: datetime, universe: list) -> pd.DataFrame:
        """Extract value factors using QVM engine methodology."""
        try:
            # Get QVM composite scores which include value factors
            qvm_scores = self.qvm_engine.calculate_qvm_composite(analysis_date, universe)
            ...
            print(f"Error extracting value factors: {e}")
            # Return zeros if QVM engine fails
            value_data = [{'ticker': ticker, 'value_score': 0} for ticker in universe]
            return pd.DataFrame(value_data)
    ...
    def print_enhanced_analysis(self, categorized_results: dict, overall_results: dict):
        """Print enhanced coefficient analysis including value factors."""
        print("\n" + "="*100)
        print("ENHANCED FACTOR COEFFICIENT ANALYSIS (WITH VALUE FACTORS)")
        print("="*100)
    ...
    def create_enhanced_visualizations(self, categorized_results: dict, overall_results: dict):
        """Create enhanced coefficient visualizations including value factors."""
        plt.style.use('seaborn-v0_8')
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Enhanced Factor Coefficient Analysis (With Value Factors)', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('enhanced_factor_analysis_with_value.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    def run_enhanced_analysis(self, analysis_date: datetime = None):
        """Run enhanced factor coefficient analysis including value factors."""
        if analysis_date is None:
            # Use a date that allows for forward returns calculation
            analysis_date = datetime(2024, 12, 18)
        
        print(f"Running enhanced factor analysis for {analysis_date}")
        ...
        # Print analysis
        self.print_enhanced_analysis(categorized_results, results)
        
        # Create visualizations
        self.create_enhanced_visualizations(categorized_results, results)
...
def main():
    ...
    results = analyzer.run_enhanced_analysis(analysis_date)
    
    if results:
        print("\nEnhanced analysis completed successfully!")
        print("Results saved to: enhanced_factor_analysis_with_value.png")
    else:
