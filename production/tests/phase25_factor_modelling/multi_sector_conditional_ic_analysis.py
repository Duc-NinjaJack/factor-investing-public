"""
Multi-Sector Conditional IC Analysis
===================================
Analyzes P/B and P/E ICs conditional on ROAA across multiple sectors
using different intermediary tables for comprehensive coverage.
"""
...
class MultiSectorConditionalICAnalyzer:
    def __init__(self):
        self.engine = get_engine()
        
    def get_multi_sector_data(self, analysis_date: datetime) -> pd.DataFrame:
        """Get comprehensive data across multiple sectors using different tables."""
        ...
        for sector in sectors:
            try:
                ...
                if len(universe) < 3:  # Need at least 3 stocks per sector
                    continue
                
                print(f"Processing sector: {sector} with {len(universe)} tickers")
                
                # Try different intermediary tables based on sector type
                sector_data = None
                
                # For financial sectors, try banking table first
                if sector in ['Banks', 'Insurance', 'Securities', 'Other Financial']:
                    sector_data = self.get_banking_data(sector, universe, analysis_date)
                
                # For non-financial sectors, try enhanced table
                if sector_data is None or len(sector_data) == 0:
                    sector_data = self.get_enhanced_data(sector, universe, analysis_date)
                
                # For securities-specific sectors, try securities table
                if sector_data is None or len(sector_data) == 0:
    ...
    def get_banking_data(self, sector: str, universe: list, analysis_date: datetime) -> pd.DataFrame:
        """Get data from banking intermediary table."""
        try:
            ...
            print(f"Error getting banking data for {sector}: {e}")
            return pd.DataFrame()
    ...
    def get_enhanced_data(self, sector: str, universe: list, analysis_date: datetime) -> pd.DataFrame:
        ...
        try:
            # Get factors for this sector from enhanced table
            factors_query = """
            SELECT ticker, 
                   NetProfit_TTM / AvgTotalAssets as roaa,
                   NetProfit_TTM / AvgTotalEquity as roae,
                   OperatingProfit_TTM / Revenue_TTM as operating_margin,
    ...
    def get_securities_data(self, sector: str, universe: list, analysis_date: datetime) -> pd.DataFrame:
        ...
        try:
            # Get factors for this sector from securities table
            factors_query = """
            SELECT ticker, 
                   NetProfit_TTM / AvgTotalAssets as roaa,
                   NetProfit_TTM / AvgTotalEquity as roae,
    ...
    def get_value_factors(self, universe: list, analysis_date: datetime) -> pd.DataFrame:
        ...
        for ticker in universe:
            try:
                # Get fundamental data from any available table
                fundamental_query = """
                SELECT 
                    NetProfit_TTM,
                    AvgTotalEquity,
    ...
    def print_sector_analysis(self, sector_overall_ics: dict, sector_conditional_ics: dict):
        ...
        print("\n" + "="*100)
        print("MULTI-SECTOR CONDITIONAL IC ANALYSIS: P/B AND P/E BY ROAA QUINTILES")
        print("="*100)
    ...
    def create_sector_visualizations(self, sector_overall_ics: dict, sector_conditional_ics: dict):
        ...
        fig.suptitle('Multi-Sector Conditional IC Analysis: P/B and P/E by ROAA Quintiles', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('multi_sector_conditional_ic_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    def run_multi_sector_analysis(self, analysis_date: datetime = None):
        """Run comprehensive multi-sector conditional IC analysis."""
        if analysis_date is None:
            analysis_date = datetime(2024, 12, 18)
        
        print(f"Running multi-sector conditional IC analysis for {analysis_date}")
        
        # Get comprehensive multi-sector data
        data = self.get_multi_sector_data(analysis_date)
        
        if len(data) == 0:
            print("No multi-sector data extracted")
            return
...
def main():
    ...
    analyzer = MultiSectorConditionalICAnalyzer()
    
    # Run analysis for date with sufficient historical data
    analysis_date = datetime(2024, 12, 18)
    
    results = analyzer.run_multi_sector_analysis(analysis_date)
    
    if results:
        print("\nMulti-sector conditional IC analysis completed successfully!")
        print("Results saved to: multi_sector_conditional_ic_analysis.png")
    else:
        print("Multi-sector conditional IC analysis failed")
