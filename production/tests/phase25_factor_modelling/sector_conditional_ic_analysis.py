...
=============================
Analyzes P/B and P/E ICs conditional on ROAA, broken down by sectors.
"""
...
class SectorConditionalICAnalyzer:
    def __init__(self):
        self.engine = get_engine()
        
    def get_sector_data(self, analysis_date: datetime) -> pd.DataFrame:
        """Get comprehensive data across all sectors."""
        # Get all sectors data from master_info
        sectors_query = """
        ...
        for sector in sectors:
            try:
                ...
                # Get factors for this sector from banking table
                factors_query = """
                SELECT ticker, 
                ...
                # Get value components for this sector
                value_data = []
                ...
                # Get forward returns for this sector
    ...
    def print_sector_analysis(self, sector_overall_ics: dict, sector_conditional_ics: dict):
        """Print comprehensive sector analysis."""
        print("\n" + "="*100)
        print("SECTOR CONDITIONAL IC ANALYSIS: P/B AND P/E BY ROAA QUINTILES")
        print("="*100)
        
        print(f"\nOVERALL SECTOR ICs:")
        print(f"{'Sector':<20} {'P/E IC':<10} {'P/B IC':<10} {'N':<5} {'Mean ROAA':<12} {'Mean Return':<12}")
        print("-" * 80)
        
        for sector, stats in sector_overall_ics.items():
            print(f"{sector:<20} {stats['pe_ic']:<10.4f} {stats['pb_ic']:<10.4f} "
        ...
        # Analyze patterns across sectors
        for component in ['pe_score', 'pb_score']:
            print(f"\n{component.upper()} PATTERNS BY SECTOR:")
            ...
            for sector, quintile_data in sector_conditional_ics.items():
                ...
                for quintile, stats in quintile_data.items():
                    if component in stats['ics']:
                        component_ics.append(stats['ics'][component])
        ...
        for component in ['pe_score', 'pb_score']:
            ...
            print(f"{'Sector':<20} {'Mean IC':<10} {'Std IC':<10} {'Range':<10} {'Min':<10} {'Max':<10}")
            print("-" * 80)
            
            for sector, stats in sorted_sectors:
                print(f"{sector:<20} {stats['mean_ic']:<10.4f} {stats['std_ic']:<10.4f} "
                      f"{stats['range']:<10.4f} {stats['min_ic']:<10.4f} {stats['max_ic']:<10.4f}")
        
        print("\n" + "="*100)
    
    def create_sector_visualizations(self, sector_overall_ics: dict, sector_conditional_ics: dict):
        """Create visualizations for sector analysis."""
        plt.style.use('seaborn-v0_8')
        
        # Create multiple subplots
        fig, axes = plt.subplots(2, 3, figsize=(20, 12))
        fig.suptitle('Sector Conditional IC Analysis: P/B and P/E by ROAA Quintiles', fontsize=16, fontweight='bold')
        ...
        bars = ax1.bar(sectors, pe_ics, color=colors, alpha=0.7)
        ...
        bars = ax2.bar(sectors, pb_ics, color=colors, alpha=0.7)
        ...
        # 4. P/E IC by ROAA Quintile (Heatmap)
        ax4 = axes[1, 0]
        quintiles = ['Q1 (Lowest)', 'Q2', 'Q3', 'Q4', 'Q5 (Highest)']
        pe_heatmap_data = []
        
        for sector in sectors:
            if sector in sector_conditional_ics:
                sector_pe_ics = []
                for quintile in quintiles:
                    if quintile in sector_conditional_ics[sector]:
                        ic = sector_conditional_ics[sector][quintile]['ics']['pe_score']
                        sector_pe_ics.append(ic)
                    else:
                        sector_pe_ics.append(np.nan)
                pe_heatmap_data.append(sector_pe_ics)
        
        if pe_heatmap_data:
            im = ax4.imshow(pe_heatmap_data, cmap='RdYlBu_r', aspect='auto')
            ax4.set_xticks(range(len(quintiles)))
            ax4.set_xticklabels([q.split()[0] for q in quintiles])
            ax4.set_yticks(range(len(sectors)))
            ax4.set_yticklabels(sectors)
            ax4.set_xlabel('ROAA Quintiles')
            ax4.set_ylabel('Sectors')
            ax4.set_title('P/E IC by Sector and ROAA Quintile')
            plt.colorbar(im, ax=ax4)
        
        # 5. P/B IC by ROAA Quintile (Heatmap)
        ax5 = axes[1, 1]
        pb_heatmap_data = []
        ...
        for sector in sectors:
            if sector in sector_conditional_ics:
                ...
                for quintile in quintiles:
                    if quintile in sector_conditional_ics[sector]:
                        ic = sector_conditional_ics[sector][quintile]['ics']['pb_score']
                        sector_pb_ics.append(ic)
                    else:
                        sector_pb_ics.append(np.nan)
                pb_heatmap_data.append(sector_pb_ics)
        
        if pb_heatmap_data:
            im = ax5.imshow(pb_heatmap_data, cmap='RdYlBu_r', aspect='auto')
            ax5.set_xticks(range(len(quintiles)))
            ax5.set_xticklabels([q.split()[0] for q in quintiles])
            ax5.set_yticks(range(len(sectors)))
            ax5.set_yticklabels(sectors)
            ax5.set_xlabel('ROAA Quintiles')
            ax5.set_ylabel('Sectors')
            ax5.set_title('P/B IC by Sector and ROAA Quintile')
            plt.colorbar(im, ax=ax5)
        
        # 6. Sector Sample Sizes
        ax6 = axes[1, 2]
        sample_sizes = [sector_overall_ics[s]['n_observations'] for s in sectors]
        
        bars = ax6.bar(sectors, sample_sizes, alpha=0.7)
        ax6.set_xlabel('Sectors')
        ax6.set_ylabel('Number of Observations')
        ax6.set_title('Sample Size by Sector')
        ...
        plt.tight_layout()
        plt.savefig('sector_conditional_ic_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    def run_sector_analysis(self, analysis_date: datetime = None):
        """Run comprehensive sector conditional IC analysis."""
        if analysis_date is None:
            analysis_date = datetime(2024, 12, 18)
        
        print(f"Running sector conditional IC analysis for {analysis_date}")
        
        # Get comprehensive sector data
        data = self.get_sector_data(analysis_date)
        ...
        # Print analysis
        self.print_sector_analysis(sector_overall_ics, sector_conditional_ics)
        
        # Create visualizations
        self.create_sector_visualizations(sector_overall_ics, sector_conditional_ics)
...
def main():
    ...
    analyzer = SectorConditionalICAnalyzer()
    
    # Run analysis for date with sufficient historical data
    analysis_date = datetime(2024, 12, 18)
    
    results = analyzer.run_sector_analysis(analysis_date)
    
    if results:
        print("\nSector conditional IC analysis completed successfully!")
        print("Results saved to: sector_conditional_ic_analysis.png")
    else:
        print("Sector conditional IC analysis failed")
