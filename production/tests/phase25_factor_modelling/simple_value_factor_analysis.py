"""
Simple Value Factor Analysis
============================
Analyzes factor coefficients including manually calculated value factors
to determine if value factors enhance strategy performance.
"""
...
class SimpleValueFactorAnalyzer:
    def __init__(self):
    ...
    def get_value_factors(self, analysis_date: datetime, universe: list) -> pd.DataFrame:
        """Calculate simple value factors based on QVM engine methodology."""
        value_data = []
        
        for ticker in universe:
            try:
                # Get fundamental data for value calculation
                fundamental_query = """
                SELECT 
                    NetProfit_TTM,
                    AvgTotalEquity,
                ...
                if len(fundamental_data) > 0:
                    ...
                    if pd.notna(row['NetProfit_TTM']) and row['NetProfit_TTM'] > 0:
                        pe_ratio = row['market_cap'] / row['NetProfit_TTM']
                        pe_score = 1 / pe_ratio if pe_ratio > 0 else 0
                    
                    # P/B ratio (inverted for value score)
                    if pd.notna(row['AvgTotalEquity']) and row['AvgTotalEquity'] > 0:
                        pb_ratio = row['market_cap'] / row['AvgTotalEquity']
                        pb_score = 1 / pb_ratio if pb_ratio > 0 else 0
                    
                    # P/S ratio (inverted for value score) - using operating income for banking
                    if pd.notna(row['TotalOperatingIncome_TTM']) and row['TotalOperatingIncome_TTM'] > 0:
                        ps_ratio = row['market_cap'] / row['TotalOperatingIncome_TTM']
    ...
    def print_value_analysis(self, categorized_results: dict, overall_results: dict):
        """Print analysis including value factor assessment."""
        print("\n" + "="*100)
        print("FACTOR COEFFICIENT ANALYSIS (WITH VALUE FACTORS)")
        print("="*100)
    ...
    def create_value_visualizations(self, categorized_results: dict, overall_results: dict):
        """Create visualizations including value factor analysis."""
        plt.style.use('seaborn-v0_8')
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Factor Coefficient Analysis (With Value Factors)', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('value_factor_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    def run_analysis(self, analysis_date: datetime = None):
        """Run factor coefficient analysis including value factors."""
        if analysis_date is None:
            # Use a date that allows for forward returns calculation
            analysis_date = datetime(2024, 12, 18)
        
        print(f"Running value factor analysis for {analysis_date}")
        ...
        value_factors = self.get_value_factors(analysis_date, universe)
        ...
        # Print analysis
        self.print_value_analysis(categorized_results, results)
        
        # Create visualizations
        self.create_value_visualizations(categorized_results, results)
...
def main():
    ...
    analyzer = SimpleValueFactorAnalyzer()
    ...
    if results:
        print("\nValue factor analysis completed successfully!")
        print("Results saved to: value_factor_analysis.png")
    else:
        print("Value factor analysis failed")
