"""
Fixed Value Factor Analysis
===========================
Correctly calculates value factors using the actual column names from the database.
"""
...
class FixedValueFactorAnalyzer:
    def __init__(self):
    ...
    def get_value_factors_fixed(self, analysis_date: datetime, universe: list) -> pd.DataFrame:
        """Calculate value factors using correct column names."""
        value_data = []
        ...
        for ticker in universe:
            try:
                ...
                if len(fundamental_data) > 0:
                    ...
                    # Get market cap from daily data using correct column names
                    market_cap_query = """
                    ...
                    # Calculate value ratios (inverted for value score)
                    pe_score = 0
                    pb_score = 0
                    ps_score = 0
                    ...
                    # P/S ratio (inverted for value score) - using operating income for banking
    ...
    def normalize_factors(self, factors_df: pd.DataFrame) -> pd.DataFrame:
        ...
        factor_columns = ['roaa', 'roae', 'operating_margin', 'asset_turnover', '1M', '3M', '6M', '12M', 'value_score']
    ...
    def categorize_coefficients(self, results: dict) -> dict:
        ...
        factor_categories = {
            ...
            'Value Factors': ['value_score_normalized']
        }
    ...
    def print_fixed_analysis(self, categorized_results: dict, overall_results: dict, value_factors_df: pd.DataFrame):
        """Print analysis including value factor assessment."""
        print("\n" + "="*100)
        print("FIXED FACTOR COEFFICIENT ANALYSIS (WITH CORRECT VALUE FACTORS)")
        print("="*100)
        ...
        print(f"\n" + "="*100)
        print("DETAILED COEFFICIENT ANALYSIS BY CATEGORY")
        print("="*100)
        ...
        for category, factors in categorized_results.items():
            if factors:
                ...
                for factor_name, stats in sorted_factors:
                    ...
                    print(f"{factor_name:25} | Coef: {stats['coefficient']:8.4f} | {direction}")
        ...
        for i, (factor_name, coef, category) in enumerate(all_factors):
            ...
            print(f"{i+1:2}. {factor_name:25} | Coef: {coef:8.4f} | {direction:8} | {category}")
        
        print("\n" + "="*100)
        
        # Value factor analysis
        if 'Value Factors' in categorized_results and categorized_results['Value Factors']:
            print("\n" + "="*100)
            print("VALUE FACTOR PERFORMANCE ANALYSIS")
            print("="*100)
            
            value_factors = categorized_results['Value Factors']
            for factor_name, stats in value_factors.items():
                direction = "POSITIVE" if stats['coefficient'] > 0 else "NEGATIVE"
                print(f"Value Factor ({factor_name}): {stats['coefficient']:.4f} ({direction})")
                
                if stats['coefficient'] > 0:
                    print("✅ Value factor ADDS to performance - higher value scores predict higher returns")
                    print("   Recommendation: Include value factor in strategy")
                else:
                    print("❌ Value factor REDUCES performance - higher value scores predict lower returns")
                    print("   Recommendation: Exclude value factor from strategy or use contrarian approach")
                
                # Compare with momentum factors
                momentum_coef = categorized_results.get('Momentum Factors', {}).get('3M', {}).get('coefficient', 0)
                if abs(stats['coefficient']) > abs(momentum_coef) * 0.5:
                    print("   💡 Value factor has meaningful impact (>50% of strongest momentum factor)")
                else:
                    print("   ⚠️  Value factor has limited impact (<50% of strongest momentum factor)")
        
        # Value factor data summary
        print("\n" + "="*100)
        print("VALUE FACTOR DATA SUMMARY")
        print("="*100)
        
        print("Value factors summary statistics:")
        print(value_factors_df[['value_score', 'pe_score', 'pb_score', 'ps_score', 'market_cap']].describe())
        
        # Check for non-zero values
        non_zero_count = (value_factors_df['value_score'] != 0).sum()
        total_count = len(value_factors_df)
        print(f"\nNon-zero value scores: {non_zero_count}/{total_count} ({non_zero_count/total_count*100:.1f}%)")
        
        if non_zero_count > 0:
            non_zero_data = value_factors_df[value_factors_df['value_score'] != 0]
            print(f"\nSample non-zero value factors:")
            print(non_zero_data[['ticker', 'value_score', 'pe_score', 'pb_score', 'ps_score', 'market_cap']].head())
        
        print("\n" + "="*100)
    
    def create_fixed_visualizations(self, categorized_results: dict, overall_results: dict):
        """Create visualizations including value factor analysis."""
        plt.style.use('seaborn-v0_8')
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Fixed Factor Coefficient Analysis (With Correct Value Factors)', fontsize=16, fontweight='bold')
        ...
        ax2.set_yticklabels(importance_names, fontsize=8)
        ...
        plt.tight_layout()
        plt.savefig('fixed_value_factor_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    def run_fixed_analysis(self, analysis_date: datetime = None):
        """Run factor coefficient analysis with corrected value factors."""
        if analysis_date is None:
            # Use a date that allows for forward returns calculation
            analysis_date = datetime(2024, 12, 18)
        
        print(f"Running fixed value factor analysis for {analysis_date}")
        ...
        value_factors = self.get_value_factors_fixed(analysis_date, universe)
        ...
        all_factors = all_factors.merge(value_factors, on='ticker', how='inner')
        ...
        # Print analysis
        self.print_fixed_analysis(categorized_results, results, value_factors)
        
        # Create visualizations
        self.create_fixed_visualizations(categorized_results, results)
...
def main():
    ...
    analyzer = FixedValueFactorAnalyzer()
    
    # Run analysis for date with sufficient historical data
    analysis_date = datetime(2024, 12, 18)
    
    results = analyzer.run_fixed_analysis(analysis_date)
    
    if results:
        print("\nFixed value factor analysis completed successfully!")
        print("Results saved to: fixed_value_factor_analysis.png")
    else:
        print("Fixed value factor analysis failed")
