#!/usr/bin/env python3
"""
Market Cap Quartile Momentum Analysis

Analyze momentum factor performance across market cap quartiles
for different time periods (2016-2020, 2021-2025).
...
def test_quartile_regime(engine, regime_name, start_date, end_date, forward_months=1):
    """Test momentum factor across market cap quartiles for a specific regime."""
    print(f"\n{'='*60}")
...
def main():
    """Main execution function for market cap quartile analysis."""
    print("🚀 MARKET CAP QUARTILE MOMENTUM ANALYSIS")
    print("=" * 60)
    ...
    for regime_name, start_date, end_date in regimes:
        ...
        for forward_months in [1, 3, 6, 12]:
            ...
            quartile_results = test_quartile_regime(
                engine, regime_name, start_date, end_date, forward_months
            )
            
            analysis_summary = analyze_quartile_results(quartile_results, f"{regime_name} ({forward_months}M)")
            
            if analysis_summary:
                key = f"{regime_name}_{forward_months}M"
                all_results[key] = analysis_summary
                
                # Create visualizations for 6M horizon (most representative)
                if forward_months == 6:
                    create_quartile_visualizations(analysis_summary, f"{regime_name} ({forward_months}M)")
    ...
    print(f"\n✅ Market cap quartile analysis completed!")
