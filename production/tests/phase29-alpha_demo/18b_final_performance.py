...
print("✅ QVM Engine v3j Final Performance (v18b)")

# Configuration
CONFIG = {
    'strategy_name': 'QVM_Engine_v3j_Final_Performance_v18b',
    'top_n_stocks': 20,
...
def calculate_final_returns(holdings_df, price_data, benchmark_data):
    """Calculate final portfolio returns with proper price handling."""
    print("📈 Calculating final portfolio returns...")
    ...
    for i, date in enumerate(unique_dates):
        ...
        # Get price data for this date and nearby dates
        date_prices = price_data[price_data['date'] == date]
        
        # If no exact date match, get the closest available price
        if date_prices.empty:
            # Get prices within 5 days of the rebalancing date
            start_date = date - pd.Timedelta(days=5)
            end_date = date + pd.Timedelta(days=5)
            
            nearby_prices = price_data[
            ...
            if not nearby_prices.empty:
                # For each ticker, get the closest price to the rebalancing date
                closest_prices = []
                for ticker in date_holdings['ticker'].unique():
                    ticker_prices = nearby_prices[nearby_prices['ticker'] == ticker]
                    if not ticker_prices.empty:
                        # Find the closest date
                        ticker_prices['date_diff'] = abs(ticker_prices['date'] - date)
                        closest_price = ticker_prices.loc[ticker_prices['date_diff'].idxmin()]
                        closest_prices.append(closest_price)
                
                if closest_prices:
                    date_prices = pd.DataFrame(closest_prices)
...
def main():
    print("🚀 Starting Final Performance Analysis")
    print("=" * 80)
    ...
    try:
        ...
        # Calculate returns
        portfolio_values, daily_returns = calculate_final_returns(holdings_df, price_data, benchmark_data)
        
        # Calculate performance metrics
        performance_metrics = calculate_performance_metrics(portfolio_values, daily_returns, benchmark_data)
        
        # Generate tearsheet
        generate_final_tearsheet(holdings_df, portfolio_values, daily_returns, performance_metrics, CONFIG)
        
        # Save results
        results_dir = Path("insights")
        results_dir.mkdir(exist_ok=True)
        
        portfolio_values.to_csv(results_dir / "18b_final_portfolio_values.csv", index=False)
        daily_returns.to_csv(results_dir / "18b_final_daily_returns.csv", index=False)
        
        # Save performance metrics
        with open(results_dir / "18b_final_performance_metrics.txt", 'w') as f:
            for metric, value in performance_metrics.items():
                f.write(f"{metric}: {value}\n")
        
        print(f"📁 Results saved to insights/")
        print(f"   - 18b_final_portfolio_values.csv: {len(portfolio_values)} portfolio values")
        print(f"   - 18b_final_daily_returns.csv: {len(daily_returns)} daily returns")
        print(f"   - 18b_final_performance_metrics.txt: Performance metrics")
...
def generate_final_tearsheet(holdings_df, portfolio_values, daily_returns, performance_metrics, config):
    ...
    print("\n✅ Final tearsheet analysis completed")
