...
print("✅ QVM Engine v3j Fixed Performance (v18b)")

# Configuration
CONFIG = {
    'strategy_name': 'QVM_Engine_v3j_Fixed_Performance_v18b',
    'top_n_stocks': 20,
...
def calculate_fixed_returns(holdings_df, price_data, benchmark_data):
    """Calculate fixed portfolio returns with proper forward filling."""
    print("📈 Calculating fixed portfolio returns with forward filling...")
    ...
    all_dates = pd.date_range(
        start=holdings_df['date'].min(),
        end=holdings_df['date'].max(),
        freq='D'
    )
    ...
    price_matrix = price_matrix.reindex(all_dates).fillna(method='ffill')
...
def main():
    ...
    try:
        ...
        # Calculate returns with forward filling
        portfolio_values, daily_returns = calculate_fixed_returns(holdings_df, price_data, benchmark_data)
        
        # Calculate performance metrics
        performance_metrics = calculate_performance_metrics(portfolio_values, daily_returns, benchmark_data)
        
        # Generate tearsheet
        generate_fixed_tearsheet(holdings_df, portfolio_values, daily_returns, performance_metrics, CONFIG)
        
        # Save results
        results_dir = Path("insights")
        results_dir.mkdir(exist_ok=True)
        
        portfolio_values.to_csv(results_dir / "18b_fixed_portfolio_values.csv", index=False)
        daily_returns.to_csv(results_dir / "18b_fixed_daily_returns.csv", index=False)
        
        # Save performance metrics
        with open(results_dir / "18b_fixed_performance_metrics.txt", 'w') as f:
            for metric, value in performance_metrics.items():
                f.write(f"{metric}: {value}\n")
        
        print(f"📁 Results saved to insights/")
        print(f"   - 18b_fixed_portfolio_values.csv: {len(portfolio_values)} portfolio values")
        print(f"   - 18b_fixed_daily_returns.csv: {len(daily_returns)} daily returns")
        print(f"   - 18b_fixed_performance_metrics.txt: Performance metrics")
...
def generate_fixed_tearsheet(holdings_df, portfolio_values, daily_returns, performance_metrics, config):
    """Generate comprehensive tearsheet."""
    print("\n📊 COMPREHENSIVE PERFORMANCE TEARSHEET (FIXED)")
    print("=" * 80)
    ...
    print("\n✅ Fixed tearsheet analysis completed")
