#!/usr/bin/env python3
"""
Working Simple Strategy - Based on analysis showing simple approaches work better
"""
...
# Configuration
STRATEGY_CONFIG = {
    "strategy_name": "Working Simple Strategy",
    "backtest_start_date": "2016-01-01",
    "backtest_end_date": "2025-07-28",
    "universe": {
        "top_n_stocks": 50,  # Reduced for simplicity
        "target_portfolio_size": 20,
        "min_volume": 100000  # Reduced volume threshold
    },
    "rebalancing": {
        "frequency": "monthly",
...
class WorkingSimpleStrategy:
    ...
    def get_top_stocks(self):
        """Get top stocks by average volume"""
        query = """
        SELECT 
            ticker,
        ...
        with self.engine.connect() as conn:
            top_stocks = pd.read_sql(query, conn, params={
    ...
    def backtest(self):
        ...
        print(f"   📊 Selected top {len(top_stocks)} stocks by volume")
        
        # Create price matrix for top stocks
        top_data = data[data['ticker'].isin(top_stocks)]
        price_data = top_data.pivot(index='date', columns='ticker', values='close')
        ...
        # Simple strategy: Equal weight top 20 stocks, rebalance monthly
        ...
        print(f"   📅 Processing {len(monthly_dates)} monthly rebalancing dates...")
        
        for i, date in enumerate(monthly_dates):
            if date not in price_data.index:
                continue
            
            # Get available stocks on this date
            available_stocks = price_data.loc[date].dropna().index.tolist()
            
            if len(available_stocks) < 10:  # Need at least 10 stocks
                continue
            
            # Select top 20 stocks (or all if less than 20)
            portfolio_size = min(20, len(available_stocks))
            portfolio = available_stocks[:portfolio_size]
            
            # Equal weights
            weights = pd.Series(1.0 / portfolio_size, index=portfolio)
            
            # Calculate returns for next month
            next_month = date + pd.Timedelta(days=30)
            if next_month in returns_matrix.index:
                # Get returns for portfolio
                period_returns = returns_matrix.loc[date:next_month, portfolio]
                if not period_returns.empty:
                    # Calculate weighted returns
                    weighted_returns = (period_returns * weights).sum(axis=1)
                    monthly_return = weighted_returns.mean()
            ...
            if i % 20 == 0:
                print(f"   📅 {date.strftime('%Y-%m')}: Portfolio={len(portfolio)} stocks")
...
• Excess Return: {metrics['excess_return']:.2%}
        ...
        filename = f"tearsheet_working_simple_{timestamp}.png"
...
def main():
    """Main execution"""
    strategy = WorkingSimpleStrategy(STRATEGY_CONFIG)
