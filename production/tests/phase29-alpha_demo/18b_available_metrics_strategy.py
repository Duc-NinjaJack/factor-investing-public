...
# %% [markdown]
# # QVM Engine v3j - Available Metrics Strategy (Version 18b)
# 
...
# 2. **Available Metrics Integration** (no F-Score):
...
print("✅ QVM Engine v3j Available Metrics Strategy (v18b) initialized")

# %%
# Configuration
QVM_CONFIG = {
    'strategy_name': 'QVM_Engine_v3j_Available_Metrics_v18b',
    'universe': {
        'lookback_days': 252,
        'top_n_stocks': 20,
...
def load_available_metrics(db_engine, date, tickers):
    """Load available metrics (no F-Score)."""
    print(f"📊 Loading available metrics for {date}")
    ...
    try:
        ...
        # 2. Load FCF Yield data (quarterly refresh)
        fcf_query = f"""
        SELECT 
            ticker,
            value as fcf_yield,
        ...
        # 3. Load price data for momentum and low vol (daily refresh)
        price_query = f"""
        SELECT 
            ticker,
            trading_date,
        ...
        print(f"   ❌ Error loading available metrics: {e}")
        return pd.DataFrame()
...
def calculate_available_factors(data_df, config):
    """Calculate available factors with proper normalization."""
    print(f"📊 Calculating available factors")
    
    try:
        # 1. Quality Factor (ROAA only) - Quarterly refresh
        ...
        # Quality composite (100% ROAA)
        data_df['Quality_Composite'] = data_df['roaa_normalized']
        ...
        print(f"   ✅ Available factors calculated for {len(data_df)} stocks")
        ...
        print(f"   ❌ Error calculating available factors: {e}")
        return data_df
...
def run_available_backtest(config, db_engine):
    """Run available metrics backtest with proper factor refresh cycles."""
    print("🚀 Starting Available Metrics Strategy Backtest")
    print("=" * 80)
    ...
    try:
        ...
        for i, date in enumerate(rebalance_dates):
            ...
            try:
                ...
                metrics_data = load_available_metrics(db_engine, date, universe)
                if metrics_data.empty:
                    continue
                
                # 3. Calculate available factors
                factors_data = calculate_available_factors(metrics_data, config)
        ...
        print(f"❌ Error running available backtest: {e}")
        return pd.DataFrame(), pd.DataFrame()

# %%
def main():
    """Main execution function."""
    print("🎯 QVM Engine v3j Available Metrics Strategy (v18b)")
    print("=" * 80)
    ...
    try:
        ...
        # Run available backtest
        holdings_df, regime_df = run_available_backtest(QVM_CONFIG, db_engine)
        ...
        if not holdings_df.empty:
            ...
            holdings_df.to_csv(results_dir / "18b_available_holdings.csv", index=False)
            regime_df.to_csv(results_dir / "18b_regime_history.csv", index=False)
