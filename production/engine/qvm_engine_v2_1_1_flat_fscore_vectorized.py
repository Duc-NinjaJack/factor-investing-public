# production/engine/qvm_engine_v2_1_1_flat_fscore_vectorized.py
import types
import pandas as pd
import numpy as np
...
def _ensure_cache(engine):
    if not hasattr(engine, "_fscore_cache"):
        # Keys: ('nf'|'bank'|'sec', year, quarter, date, tuple(sorted(tickers))) -> dict[ticker->int]
...
def _fetch_nf_bal_snapshot(conn, tickers, y, q):
    # Prefer a materialized snapshot table if you create it (faster, no view expansion).
    # Fallback to the view if snapshot not yet available.
    try:
        sql = text("""
...
def _compute_nf_vectorized(engine, tickers, y, q, analysis_date):
    # Cached by exact tickerset; good enough because the engine repeats with same set
    ...
    cfo      = df["NetCFO_TTM"]
    # Piotroski P4: ACCRUAL -> 1 if CFO > Net Income (not scaled)
    ...
    # splits/bonus tolerant no-issuance (1% tolerance)
    no_iss   = (df["shares"].fillna(np.inf) <= 1.01*df["shares_prev"].fillna(-np.inf)).astype(int)
...
def _compute_bank_vectorized(engine, tickers, y, q):
    ...
    with engine.engine.begin() as conn:
        ...
        prv = pd.read_sql(sql, conn, params={"y": y-1, "q": q, "tickers": tuple(tickers)})

        # balance snapshot (debt leverage proxies)
        sqlb = text("""
            SELECT ticker, TotalEquity AS ShareholdersEquity, 0 AS CustomerDeposits
            FROM v_comprehensive_fundamental_items
            WHERE year=:y AND quarter=:q AND ticker IN :tickers
        """)
    ...
    df = cur.merge(prv, on="ticker", how="inner")
    ...
    nim_prev = df["NII_TTM_prev"] / df["AvgEarningAssets_prev"].replace(0, np.nan)
    cir = df["OperatingExpenses_TTM"] / df["TotalOperatingIncome_TTM"].replace(0, np.nan)
    cir_prev = df["OperatingExpenses_TTM_prev"] / df["TotalOperatingIncome_TTM_prev"].replace(0, np.nan)
    lev = df["CustomerDeposits"] / df["ShareholdersEquity"].replace(0, np.nan)
    lev_prev = df["CustomerDeposits_prev"] / df["ShareholdersEquity_prev"].replace(0, np.nan)
    ...
    score = (
        ...
        (lev < lev_prev).astype(int) +
        (df["NetProfit_TTM"] > df["NetProfit_TTM_prev"]).astype(int)
    )
    res = dict(zip(df["ticker"], score.fillna(0).astype(int)))
    cache[key] = res
    return res
...
# -----------------------
def install_vectorized_fscore(engine):
    """Bind vectorized, cached F-Score methods onto an existing engine instance."""
...
def prime_fscore_cache(engine, universe_df, analysis_date, y, q):
    """
    Optional: warm up cache once per date with a single DB pass per sector group.
    universe_df must include ['ticker','sector']; sectors: 'Banking','Securities', else Non-Financial.
    """
    _ensure_cache(engine)
    tick = universe_df["ticker"].tolist()
    sec_map = universe_df.set_index("ticker")["sector"].to_dict()
    nf   = [t for t in tick if sec_map.get(t) not in ("Banking", "Securities")]
    bank = [t for t in tick if sec_map.get(t) == "Banking"]
